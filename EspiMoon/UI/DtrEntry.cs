...
public sealed class DtrEntry : IDisposable, IHostedService
{
    ...
    private IDtrBarEntry CreateEntry()
    {
        ...
        var entry = _dtrBar.Get("Espi Moon");
    ...
    private void Update()
    {
        ...
        if (_apiController.IsConnected)
        {
            ...
            if (pairCount > 0)
            {
                ...
                tooltip = $"Espi Moon: Connected{Environment.NewLine}----------{Environment.NewLine}{string.Join(Environment.NewLine, visiblePairs)}";
                colors = _configService.Current.DtrColorsPairsInRange;
            }
            else
            {
                tooltip = "Espi Moon: Connected";
        ...
        {
            text = "\uE044 \uE04C";
            tooltip = "Espi Moon: Not Connected";
