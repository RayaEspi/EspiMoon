...
using EspiMoon.API.Data.Extensions;
using EspiMoon.API.Dto.Group;
using EspiMoon.Interop.Ipc;
using EspiMoon.MareConfiguration;
using EspiMoon.PlayerData.Handlers;
...
using EspiMoon.UI.Components;
using EspiMoon.UI.Handlers;
using EspiMoon.WebAPI;
using EspiMoon.WebAPI.Files;
using EspiMoon.WebAPI.Files.Models;
using EspiMoon.WebAPI.SignalR;
using EspiMoon.WebAPI.SignalR.Utils;
...
public class CompactUi : WindowMediatorSubscriberBase
{
    ...
    public CompactUi(ILogger<CompactUi> logger, UiSharedService uiShared, MareConfigService configService, ApiController apiController, PairManager pairManager,
        ...
        : base(logger, mediator, "###EspiMoonMainUI", performanceCollectorService)
    {
        ...
        WindowName = $"Espi Moon {dev} ({ver.Major}.{ver.Minor}.{ver.Build})###EspiMoonMainUI";
        Toggle();
#else
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        WindowName = "Espi Moon " + ver.Major + "." + ver.Minor + "." + ver.Build + "###EspiMoonMainUI";
#endif
    ...
    protected override void DrawInternal()
    {
        ...
        if (!_apiController.IsCurrentVersion)
        {
            ...
            UiSharedService.ColorTextWrapped($"Your Espi Moon installation is out of date, the current version is {ver.Major}.{ver.Minor}.{ver.Build}. " +
                $"It is highly recommended to keep Espi Moon up to date. Open /xlplugins and update the plugin.", ImGuiColors.DalamudRed);
        }
    ...
    private string GetServerError()
    {
        return _apiController.ServerState switch
        {
            ...
            ServerState.Disconnected => "You are currently disconnected from the Espi Moon server.",
            ServerState.Disconnecting => "Disconnecting from the server",
            ServerState.Unauthorized => "Server Response: " + _apiController.AuthFailureMessage,
            ServerState.Offline => "Your selected Espi Moon server is currently offline.",
