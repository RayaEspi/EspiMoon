...
using EspiMoon.FileCache;
using EspiMoon.Localization;
using EspiMoon.MareConfiguration;
using EspiMoon.MareConfiguration.Models;
using EspiMoon.Services;
...
public partial class IntroUi : WindowMediatorSubscriberBase
{
    ...
    public IntroUi(ILogger<IntroUi> logger, UiSharedService uiShared, MareConfigService configService,
        ...
        PerformanceCollectorService performanceCollectorService, DalamudUtilService dalamudUtilService) : base(logger, mareMediator, "Espi Moon Setup", performanceCollectorService)
    {
        _uiShared = uiShared;
    ...
    protected override void DrawInternal()
    {
        ...
        {
            _uiShared.BigText("Welcome to Espi Moon");
            ImGui.Separator();
            UiSharedService.TextWrapped("Espi Moon is a plugin that will replicate your full current character state including all Penumbra mods to other paired Espi Moon users. " +
                     ...
                     || !Directory.Exists(_configService.Current.CacheFolder)))
        {
            ...
            {
                UiSharedService.TextWrapped("To not unnecessary download files already present on your computer, Espi Moon will have to scan your Penumbra mod directory. " +
                                     "Additionally, a local storage folder must be set where Espi Moon will download other character files to. " +
                                     "Once the storage folder is set and the scan complete, this page will automatically forward to registration at a service.");
                UiSharedService.TextWrapped("Note: The initial scan, depending on the amount of mods you have, might take a while. Please wait until it is completed.");
                UiSharedService.ColorTextWrapped("Warning: once past this step you should not delete the FileCache.csv of Espi Moon in the Plugin Configurations folder of Dalamud. " +
        ...
        else if (!_uiShared.ApiController.ServerAlive)
        {
            ...
            ImGui.Separator();
            UiSharedService.TextWrapped("To be able to use Espi Moon you will have to register an account.");
            UiSharedService.TextWrapped("For the official Espi Moon Servers the account creation will be handled on the official Espi Moon Discord. " +
                              "You will have to join the discord and use the bot in the #moon-service channel to register an account. " +
                              "After you have registered you can come back here and connect to the service.");

            // Button to open the discord
            if (ImGui.Button("Open Espi Moon Discord"))
            {
                Util.OpenLink("https://discord.gg/UZHF4smjYA");
            }
            
            UiSharedService.TextWrapped("To join other servers, contact the appropriate service provider how to obtain a secret key, usually through a discord bot in their discord servers.");
