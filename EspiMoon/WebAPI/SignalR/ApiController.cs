...
using EspiMoon.API.Data;
using EspiMoon.API.Data.Extensions;
using EspiMoon.API.Dto;
using EspiMoon.API.Dto.User;
using EspiMoon.API.SignalR;
...
namespace EspiMoon.WebAPI.SignalR;

#pragma warning disable MA0040
public sealed partial class ApiController : DisposableMediatorSubscriberBase, IMareHubClient
{
    public const string MainServer = "Espi Moon (Main)";
    public const string MainServiceUri = "wss://mare.espi-couple.com";
            ...
            try
            {
                ...
                if (_connectionDto.ServerVersion != IMareHub.ApiVersion)
                {
                    if (_connectionDto.CurrentClientVersion > currentClientVer)
                    {
                        Mediator.Publish(new NotificationMessage("Client incompatible",
                            ...
                            $"This client version is incompatible and will not be able to connect. Please update your Espi Moon client.",
                            NotificationType.Error));
                    }
                ...
                if (_connectionDto.CurrentClientVersion > currentClientVer)
                {
                    Mediator.Publish(new NotificationMessage("Client outdated",
                        $"Your client is outdated ({currentClientVer.Major}.{currentClientVer.Minor}.{currentClientVer.Build}), current is: " +
                        $"{_connectionDto.CurrentClientVersion.Major}.{_connectionDto.CurrentClientVersion.Minor}.{_connectionDto.CurrentClientVersion.Build}. " +
                        $"Please keep your Espi Moon client up-to-date.",
                ...
                if (_dalamudUtil.HasModifiedGameFiles)
                {
                    ...
                    if (!_mareConfigService.Current.DebugStopWhining)
                        Mediator.Publish(new NotificationMessage("Modified Game Files detected",
                            ...
                            "Espi Moon, Penumbra, and some other plugins assume your FFXIV installation is unmodified in order to work. " +
