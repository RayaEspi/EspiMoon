...
using NotificationType = EspiMoon.MareConfiguration.Models.NotificationType;
...
public class NotificationService : DisposableMediatorSubscriberBase, IHostedService
{
    ...
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Espi Moon] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Espi Moon] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Espi Moon] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
